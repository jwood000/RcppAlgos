...
do {                                                     \
    Rcpp::stop("libdivide.h:%d: %s(): Error: %s\n", \
...
do {                                                                    \
    if (!(x)) {                                                         \
        Rcpp::stop("libdivide.h:%d: %s(): Assertion failed: %s\n", \
...
static inline struct libdivide_s32_t libdivide_s32_gen(std::int32_t d);
static inline struct libdivide_u32_t libdivide_u32_gen(std::uint32_t d);
static inline struct libdivide_s64_t libdivide_s64_gen(std::int64_t d);
static inline struct libdivide_u64_t libdivide_u64_gen(std::uint64_t d);

static inline struct libdivide_s32_branchfree_t libdivide_s32_branchfree_gen(std::int32_t d);
static inline struct libdivide_u32_branchfree_t libdivide_u32_branchfree_gen(std::uint32_t d);
static inline struct libdivide_s64_branchfree_t libdivide_s64_branchfree_gen(std::int64_t d);
static inline struct libdivide_u64_branchfree_t libdivide_u64_branchfree_gen(std::uint64_t d);

static inline std::int32_t  libdivide_s32_do(std::int32_t numer, const struct libdivide_s32_t *denom);
static inline std::uint32_t libdivide_u32_do(std::uint32_t numer, const struct libdivide_u32_t *denom);
static inline std::int64_t  libdivide_s64_do(std::int64_t numer, const struct libdivide_s64_t *denom);
static inline std::uint64_t libdivide_u64_do(std::uint64_t numer, const struct libdivide_u64_t *denom);

static inline std::int32_t  libdivide_s32_branchfree_do(std::int32_t numer, const struct libdivide_s32_branchfree_t *denom);
static inline std::uint32_t libdivide_u32_branchfree_do(std::uint32_t numer, const struct libdivide_u32_branchfree_t *denom);
static inline std::int64_t  libdivide_s64_branchfree_do(std::int64_t numer, const struct libdivide_s64_branchfree_t *denom);
static inline std::uint64_t libdivide_u64_branchfree_do(std::uint64_t numer, const struct libdivide_u64_branchfree_t *denom);

static inline std::int32_t  libdivide_s32_recover(const struct libdivide_s32_t *denom);
static inline std::uint32_t libdivide_u32_recover(const struct libdivide_u32_t *denom);
static inline std::int64_t  libdivide_s64_recover(const struct libdivide_s64_t *denom);
static inline std::uint64_t libdivide_u64_recover(const struct libdivide_u64_t *denom);

static inline std::int32_t  libdivide_s32_branchfree_recover(const struct libdivide_s32_branchfree_t *denom);
static inline std::uint32_t libdivide_u32_branchfree_recover(const struct libdivide_u32_branchfree_t *denom);
static inline std::int64_t  libdivide_s64_branchfree_recover(const struct libdivide_s64_branchfree_t *denom);
static inline std::uint64_t libdivide_u64_branchfree_recover(const struct libdivide_u64_branchfree_t *denom);

//////// Internal Utility Functions

static inline std::uint32_t libdivide_mullhi_u32(std::uint32_t x, std::uint32_t y) {
    std::uint64_t xl = x, yl = y;
    std::uint64_t rl = xl * yl;
    return (std::uint32_t)(rl >> 32);
}

static inline std::int32_t libdivide_mullhi_s32(std::int32_t x, std::int32_t y) {
    std::int64_t xl = x, yl = y;
    std::int64_t rl = xl * yl;
    // needs to be arithmetic shift
    return (std::int32_t)(rl >> 32);
}

static inline std::uint64_t libdivide_mullhi_u64(std::uint64_t x, std::uint64_t y) {
    ...
    return (std::uint64_t)(rl >> 64);
#else
    // full 128 bits are x0 * y0 + (x0 * y1 << 32) + (x1 * y0 << 32) + (x1 * y1 << 64)
    std::uint32_t mask = 0xFFFFFFFF;
    std::uint32_t x0 = (std::uint32_t)(x & mask);
    std::uint32_t x1 = (std::uint32_t)(x >> 32);
    std::uint32_t y0 = (std::uint32_t)(y & mask);
    std::uint32_t y1 = (std::uint32_t)(y >> 32);
    std::uint32_t x0y0_hi = libdivide_mullhi_u32(x0, y0);
    std::uint64_t x0y1 = x0 * (std::uint64_t)y1;
    std::uint64_t x1y0 = x1 * (std::uint64_t)y0;
    std::uint64_t x1y1 = x1 * (std::uint64_t)y1;
    std::uint64_t temp = x1y0 + x0y0_hi;
    std::uint64_t temp_lo = temp & mask;
    std::uint64_t temp_hi = temp >> 32;
    
    return x1y1 + temp_hi + ((temp_lo + x0y1) >> 32);
#endif
}

static inline std::int64_t libdivide_mullhi_s64(std::int64_t x, std::int64_t y) {
#if defined(LIBDIVIDE_VC) && \
    defined(LIBDIVIDE_X86_64)
    return __mulh(x, y);
#elif defined(HAS_INT128_T)
    ...
    __int128_t rl = xl * yl;
    return (std::int64_t)(rl >> 64);
#else
    // full 128 bits are x0 * y0 + (x0 * y1 << 32) + (x1 * y0 << 32) + (x1 * y1 << 64)
    std::uint32_t mask = 0xFFFFFFFF;
    std::uint32_t x0 = (std::uint32_t)(x & mask);
    std::uint32_t y0 = (std::uint32_t)(y & mask);
    std::int32_t x1 = (std::int32_t)(x >> 32);
    std::int32_t y1 = (std::int32_t)(y >> 32);
    std::uint32_t x0y0_hi = libdivide_mullhi_u32(x0, y0);
    std::int64_t t = x1 * (std::int64_t)y0 + x0y0_hi;
    std::int64_t w1 = x0 * (std::int64_t)y1 + (t & mask);
    
    return x1 * (std::int64_t)y1 + (t >> 32) + (w1 >> 32);
#endif
}

static inline std::int32_t libdivide_count_leading_zeros32(std::uint32_t val) {
#if defined(__GNUC__) || \
...
static inline std::int32_t libdivide_count_leading_zeros64(std::uint64_t val) {
#if defined(__GNUC__) || \
    ...
    std::uint32_t lo = val & 0xFFFFFFFF;
...
static inline std::uint32_t libdivide_64_div_32_to_32(std::uint32_t u1, std::uint32_t u0, std::uint32_t v, std::uint32_t *r) {
    ...
    return result;
#else
    std::uint64_t n = ((std::uint64_t)u1 << 32) | u0;
    std::uint32_t result = (std::uint32_t)(n / v);
    *r = (std::uint32_t)(n - result * (std::uint64_t)v);
    return result;
#endif
}
...
// Returns the quotient directly and the remainder in *r
static std::uint64_t libdivide_128_div_64_to_64(std::uint64_t u1, std::uint64_t u0, std::uint64_t v, std::uint64_t *r) {
    ...
    std::uint64_t result = (std::uint64_t)(n / v);
    *r = (std::uint64_t)(n - result * (__uint128_t)v);
    return result;
#else
    ...
    const std::uint64_t b = (1ULL << 32); // Number base (32 bits)
    std::uint64_t un1, un0; // Norm. dividend LSD's
    std::uint64_t vn1, vn0; // Norm. divisor digits
    std::uint64_t q1, q0; // Quotient digits
    std::uint64_t un64, un21, un10; // Dividend digit pairs
    std::uint64_t rhat; // A remainder
    std::int32_t s; // Shift amount for norm
...
static inline void libdivide_u128_shift(std::uint64_t *u1, std::uint64_t *u0, std::int32_t signed_shift) {
    if (signed_shift > 0) {
        std::uint32_t shift = signed_shift;
        *u1 <<= shift;
        *u1 |= *u0 >> (64 - shift);
        *u0 <<= shift;
    }
...
static std::uint64_t libdivide_128_div_128_to_64(std::uint64_t u_hi, std::uint64_t u_lo, std::uint64_t v_hi, std::uint64_t v_lo, std::uint64_t *r_hi, std::uint64_t *r_lo) {
    ...
    ufull = (ufull << 64) | u_lo;
    vfull = (vfull << 64) | v_lo;
    std::uint64_t res = (std::uint64_t)(ufull / vfull);
    __uint128_t remainder = ufull - (vfull * res);
    *r_lo = (std::uint64_t)remainder;
    *r_hi = (std::uint64_t)(remainder >> 64);
    return res;
#else
    // Adapted from "Unsigned Doubleword Division" in Hacker's Delight
    // We want to compute u / v
    typedef struct { std::uint64_t hi; std::uint64_t lo; } u128_t;
    u128_t u = {u_hi, u_lo};
    u128_t v = {v_hi, v_lo};
    ...
    // We have 0 <= n <= 63
    std::uint32_t n = libdivide_count_leading_zeros64(v.hi);
    
    // Normalize the divisor so its MSB is 1
    u128_t v1t = v;
    libdivide_u128_shift(&v1t.hi, &v1t.lo, n);
    std::uint64_t v1 = v1t.hi; // i.e. v1 = v1t >> 64
    ...
    std::uint64_t rem_ignored;
    std::uint64_t q1 = libdivide_128_div_64_to_64(u1.hi, u1.lo, v1, &rem_ignored);
...
static inline struct libdivide_u32_t libdivide_internal_u32_gen(std::uint32_t d, int branchfree) {
    if (d == 0) {
        LIBDIVIDE_ERROR("divider must be != 0");
    }
    
    struct libdivide_u32_t result;
    std::uint32_t floor_log_2_d = 31 - libdivide_count_leading_zeros32(d);
    ...
    if ((d & (d - 1)) == 0) {
        ...
        result.magic = 0;
        result.more = (std::uint8_t)(floor_log_2_d - (branchfree != 0));
    } else {
        std::uint8_t more;
        std::uint32_t rem, proposed_m;
        ...
        if (!branchfree && (e < (1U << floor_log_2_d))) {
            ...
            const std::uint32_t twice_rem = rem + rem;
...
struct libdivide_u32_t libdivide_u32_gen(std::uint32_t d) {
    return libdivide_internal_u32_gen(d, 0);
}

struct libdivide_u32_branchfree_t libdivide_u32_branchfree_gen(std::uint32_t d) {
    if (d == 1) {
        LIBDIVIDE_ERROR("branchfree divider must be != 1");
    }
    struct libdivide_u32_t tmp = libdivide_internal_u32_gen(d, 1);
    struct libdivide_u32_branchfree_t ret = {tmp.magic, (std::uint8_t)(tmp.more & LIBDIVIDE_32_SHIFT_MASK)};
    return ret;
}

std::uint32_t libdivide_u32_do(std::uint32_t numer, const struct libdivide_u32_t *denom) {
    std::uint8_t more = denom->more;
    if (!denom->magic) {
        return numer >> more;
    }
    else {
        std::uint32_t q = libdivide_mullhi_u32(denom->magic, numer);
        if (more & LIBDIVIDE_ADD_MARKER) {
            std::uint32_t t = ((numer - q) >> 1) + q;
...
std::uint32_t libdivide_u32_branchfree_do(std::uint32_t numer, const struct libdivide_u32_branchfree_t *denom) {
    std::uint32_t q = libdivide_mullhi_u32(denom->magic, numer);
    std::uint32_t t = ((numer - q) >> 1) + q;
    return t >> denom->more;
}

std::uint32_t libdivide_u32_recover(const struct libdivide_u32_t *denom) {
    std::uint8_t more = denom->more;
    std::uint8_t shift = more & LIBDIVIDE_32_SHIFT_MASK;
    ...
    if (!denom->magic) {
        ...
        std::uint32_t hi_dividend = 1U << shift;
        std::uint32_t rem_ignored;
        ...
        std::uint64_t half_n = 1ULL << (32 + shift);
        std::uint64_t d = (1ULL << 32) | denom->magic;
        // Note that the quotient is guaranteed <= 32 bits, but the remainder
        // may need 33!
        std::uint32_t half_q = (std::uint32_t)(half_n / d);
        std::uint64_t rem = half_n % d;
        // We computed 2^(32+shift)/(m+2^32)
        // Need to double it, and then add 1 to the quotient if doubling th
        // remainder would increase the quotient.
        // Note that rem<<1 cannot overflow, since rem < d and d is 33 bits
        std::uint32_t full_q = half_q + half_q + ((rem<<1) >= d);
...
std::uint32_t libdivide_u32_branchfree_recover(const struct libdivide_u32_branchfree_t *denom) {
    std::uint8_t more = denom->more;
    std::uint8_t shift = more & LIBDIVIDE_32_SHIFT_MASK;
...
static inline struct libdivide_u64_t libdivide_internal_u64_gen(std::uint64_t d, int branchfree) {
    if (d == 0) {
        LIBDIVIDE_ERROR("divider must be != 0");
    }
    
    struct libdivide_u64_t result;
    std::uint32_t floor_log_2_d = 63 - libdivide_count_leading_zeros64(d);
...
struct libdivide_u64_t libdivide_u64_gen(std::uint64_t d) {
    return libdivide_internal_u64_gen(d, 0);
}

struct libdivide_u64_branchfree_t libdivide_u64_branchfree_gen(std::uint64_t d) {
    if (d == 1) {
        LIBDIVIDE_ERROR("branchfree divider must be != 1");
    }
    struct libdivide_u64_t tmp = libdivide_internal_u64_gen(d, 1);
    struct libdivide_u64_branchfree_t ret = {tmp.magic, (std::uint8_t)(tmp.more & LIBDIVIDE_64_SHIFT_MASK)};
    return ret;
}

std::uint64_t libdivide_u64_do(std::uint64_t numer, const struct libdivide_u64_t *denom) {
    std::uint8_t more = denom->more;
    if (!denom->magic) {
        return numer >> more;
    }
    else {
        std::uint64_t q = libdivide_mullhi_u64(denom->magic, numer);
        if (more & LIBDIVIDE_ADD_MARKER) {
            std::uint64_t t = ((numer - q) >> 1) + q;
...
std::uint64_t libdivide_u64_branchfree_do(std::uint64_t numer, const struct libdivide_u64_branchfree_t *denom) {
    std::uint64_t q = libdivide_mullhi_u64(denom->magic, numer);
    std::uint64_t t = ((numer - q) >> 1) + q;
    return t >> denom->more;
}

std::uint64_t libdivide_u64_recover(const struct libdivide_u64_t *denom) {
    std::uint8_t more = denom->more;
    std::uint8_t shift = more & LIBDIVIDE_64_SHIFT_MASK;
    ...
    if (!denom->magic) {
        ...
        // We know d is not a power of 2, so m is not a power of 2,
        // so we can just add 1 to the floor
        std::uint64_t hi_dividend = 1ULL << shift;
        std::uint64_t rem_ignored;
        ...
        std::uint64_t half_n_hi = 1ULL << shift, half_n_lo = 0;
        // d is a 65 bit value. The high bit is always set to 1.
        const std::uint64_t d_hi = 1, d_lo = denom->magic;
        // Note that the quotient is guaranteed <= 64 bits,
        // but the remainder may need 65!
        std::uint64_t r_hi, r_lo;
        std::uint64_t half_q = libdivide_128_div_128_to_64(half_n_hi, half_n_lo, d_hi, d_lo, &r_hi, &r_lo);
        // We computed 2^(64+shift)/(m+2^64)
        // Double the remainder ('dr') and check if that is larger than d
        // Note that d is a 65 bit value, so r1 is small and so r1 + r1
        // cannot overflow
        std::uint64_t dr_lo = r_lo + r_lo;
        std::uint64_t dr_hi = r_hi + r_hi + (dr_lo < r_lo); // last term is carry
        int dr_exceeds_d = (dr_hi > d_hi) || (dr_hi == d_hi && dr_lo >= d_lo);
        std::uint64_t full_q = half_q + half_q + (dr_exceeds_d ? 1 : 0);
        return full_q + 1;
    }
}

std::uint64_t libdivide_u64_branchfree_recover(const struct libdivide_u64_branchfree_t *denom) {
    std::uint8_t more = denom->more;
    std::uint8_t shift = more & LIBDIVIDE_64_SHIFT_MASK;
...
static inline struct libdivide_s32_t libdivide_internal_s32_gen(std::int32_t d, int branchfree) {
    if (d == 0) {
        LIBDIVIDE_ERROR("divider must be != 0");
    }
    ...
    // and is a power of 2.
    std::uint32_t ud = (std::uint32_t)d;
    std::uint32_t absD = (d < 0) ? -ud : ud;
    std::uint32_t floor_log_2_d = 31 - libdivide_count_leading_zeros32(absD);
    ...
    if ((absD & (absD - 1)) == 0) {
        ...
        proposed_m += 1;
        std::int32_t magic = (std::int32_t)proposed_m;
...
struct libdivide_s32_t libdivide_s32_gen(std::int32_t d) {
    return libdivide_internal_s32_gen(d, 0);
}

struct libdivide_s32_branchfree_t libdivide_s32_branchfree_gen(std::int32_t d) {
    struct libdivide_s32_t tmp = libdivide_internal_s32_gen(d, 1);
    struct libdivide_s32_branchfree_t result = {tmp.magic, tmp.more};
    return result;
}

std::int32_t libdivide_s32_do(std::int32_t numer, const struct libdivide_s32_t *denom) {
    std::uint8_t more = denom->more;
    std::uint8_t shift = more & LIBDIVIDE_32_SHIFT_MASK;
    
    if (!denom->magic) {
        std::uint32_t sign = (std::int8_t)more >> 7;
        std::uint32_t mask = (1U << shift) - 1;
        std::uint32_t uq = numer + ((numer >> 31) & mask);
        std::int32_t q = (std::int32_t)uq;
        q >>= shift;
        q = (q ^ sign) - sign;
        return q;
    } else {
        std::uint32_t uq = (std::uint32_t)libdivide_mullhi_s32(denom->magic, numer);
        if (more & LIBDIVIDE_ADD_MARKER) {
            // must be arithmetic shift and then sign extend
            std::int32_t sign = (std::int8_t)more >> 7;
            // q += (more < 0 ? -numer : numer)
            // cast required to avoid UB
            uq += ((std::uint32_t)numer ^ sign) - sign;
        }
        std::int32_t q = (std::int32_t)uq;
        q >>= shift;
        q += (q < 0);
...
std::int32_t libdivide_s32_branchfree_do(std::int32_t numer, const struct libdivide_s32_branchfree_t *denom) {
    std::uint8_t more = denom->more;
    std::uint8_t shift = more & LIBDIVIDE_32_SHIFT_MASK;
    // must be arithmetic shift and then sign extend
    std::int32_t sign = (std::int8_t)more >> 7;
    std::int32_t magic = denom->magic;
    std::int32_t q = libdivide_mullhi_s32(magic, numer);
    q += numer;
    
    // If q is non-negative, we have nothing to do
    // If q is negative, we want to add either (2**shift)-1 if d is a power of
    // 2, or (2**shift) if it is not a power of 2
    std::uint32_t is_power_of_2 = (magic == 0);
    std::uint32_t q_sign = (std::uint32_t)(q >> 31);
...
std::int32_t libdivide_s32_recover(const struct libdivide_s32_t *denom) {
    std::uint8_t more = denom->more;
    std::uint8_t shift = more & LIBDIVIDE_32_SHIFT_MASK;
    if (!denom->magic) {
        std::uint32_t absD = 1U << shift;
        if (more & LIBDIVIDE_NEGATIVE_DIVISOR) {
            absD = -absD;
        }
        return (std::int32_t)absD;
    } else {
        ...
        if (denom->magic == 0) {
            std::int32_t result = 1U << shift;
            return negative_divisor ? -result : result;
        }
        
        std::uint32_t d = (std::uint32_t)(magic_was_negated ? -denom->magic : denom->magic);
        std::uint64_t n = 1ULL << (32 + shift); // this shift cannot exceed 30
        std::uint32_t q = (std::uint32_t)(n / d);
        std::int32_t result = (std::int32_t)q;
        result += 1;
        return negative_divisor ? -result : result;
    }
}

std::int32_t libdivide_s32_branchfree_recover(const struct libdivide_s32_branchfree_t *denom) {
    return libdivide_s32_recover((const struct libdivide_s32_t *)denom);
}

///////////// SINT64

static inline struct libdivide_s64_t libdivide_internal_s64_gen(std::int64_t d, int branchfree) {
    if (d == 0) {
        LIBDIVIDE_ERROR("divider must be != 0");
    }
    ...
    std::uint64_t absD = (d < 0) ? -ud : ud;
    std::uint32_t floor_log_2_d = 63 - libdivide_count_leading_zeros64(absD);
    // check if exactly one bit is set,
    // don't care if absD is 0 since that's divide by zero
    if ((absD & (absD - 1)) == 0) {
        ...
        // is 0 and the high word is floor_log_2_d - 1
        std::uint8_t more;
        std::uint64_t rem, proposed_m;
        proposed_m = libdivide_128_div_64_to_64(1ULL << (floor_log_2_d - 1), 0, absD, &rem);
        const std::uint64_t e = absD - rem;
        ...
        proposed_m += 1;
        std::int64_t magic = (std::int64_t)proposed_m;
...
struct libdivide_s64_t libdivide_s64_gen(std::int64_t d) {
    return libdivide_internal_s64_gen(d, 0);
}

struct libdivide_s64_branchfree_t libdivide_s64_branchfree_gen(std::int64_t d) {
    struct libdivide_s64_t tmp = libdivide_internal_s64_gen(d, 1);
    struct libdivide_s64_branchfree_t ret = {tmp.magic, tmp.more};
    return ret;
}

std::int64_t libdivide_s64_do(std::int64_t numer, const struct libdivide_s64_t *denom) {
    std::uint8_t more = denom->more;
    std::uint8_t shift = more & LIBDIVIDE_64_SHIFT_MASK;
    
    if (!denom->magic) { // shift path
        std::uint64_t mask = (1ULL << shift) - 1;
        std::uint64_t uq = numer + ((numer >> 63) & mask);
        std::int64_t q = (std::int64_t)uq;
        q >>= shift;
        // must be arithmetic shift and then sign-extend
        std::int64_t sign = (std::int8_t)more >> 7;
        q = (q ^ sign) - sign;
        return q;
    } else {
        std::uint64_t uq = (std::uint64_t)libdivide_mullhi_s64(denom->magic, numer);
        if (more & LIBDIVIDE_ADD_MARKER) {
            // must be arithmetic shift and then sign extend
            std::int64_t sign = (std::int8_t)more >> 7;
            // q += (more < 0 ? -numer : numer)
            // cast required to avoid UB
            uq += ((std::uint64_t)numer ^ sign) - sign;
        }
        std::int64_t q = (std::int64_t)uq;
        q >>= shift;
        q += (q < 0);
...
std::int64_t libdivide_s64_branchfree_do(std::int64_t numer, const struct libdivide_s64_branchfree_t *denom) {
    std::uint8_t more = denom->more;
    std::uint8_t shift = more & LIBDIVIDE_64_SHIFT_MASK;
    // must be arithmetic shift and then sign extend
    std::int64_t sign = (std::int8_t)more >> 7;
    std::int64_t magic = denom->magic;
    std::int64_t q = libdivide_mullhi_s64(magic, numer);
    q += numer;
    
    // If q is non-negative, we have nothing to do.
    // If q is negative, we want to add either (2**shift)-1 if d is a power of
    // 2, or (2**shift) if it is not a power of 2.
    std::uint64_t is_power_of_2 = (magic == 0);
    std::uint64_t q_sign = (std::uint64_t)(q >> 63);
...
std::int64_t libdivide_s64_recover(const struct libdivide_s64_t *denom) {
    std::uint8_t more = denom->more;
    std::uint8_t shift = more & LIBDIVIDE_64_SHIFT_MASK;
    if (denom->magic == 0) { // shift path
        std::uint64_t absD = 1ULL << shift;
        if (more & LIBDIVIDE_NEGATIVE_DIVISOR) {
            absD = -absD;
        }
        return (std::int64_t)absD;
    } else {
        // Unsigned math is much easier
        int negative_divisor = (more & LIBDIVIDE_NEGATIVE_DIVISOR);
        int magic_was_negated = (more & LIBDIVIDE_ADD_MARKER)
        ...
        std::uint64_t d = (std::uint64_t)(magic_was_negated ? -denom->magic : denom->magic);
        std::uint64_t n_hi = 1ULL << shift, n_lo = 0;
        std::uint64_t rem_ignored;
        std::uint64_t q = libdivide_128_div_64_to_64(n_hi, n_lo, d, &rem_ignored);
        std::int64_t result = (std::int64_t)(q + 1);
        if (negative_divisor) {
            result = -result;
        }
        return result;
...
std::int64_t libdivide_s64_branchfree_recover(const struct libdivide_s64_branchfree_t *denom) {
...
__m512i libdivide_u32_do_vector(__m512i numers, const struct libdivide_u32_t *denom) {
    ...
    if (!denom->magic) {
        ...
        if (more & LIBDIVIDE_ADD_MARKER) {
            ...
            std::uint32_t shift = more & LIBDIVIDE_32_SHIFT_MASK;
...
__m512i libdivide_u64_do_vector(__m512i numers, const struct libdivide_u64_t *denom) {
    ...
    if (!denom->magic) {
        ...
        if (more & LIBDIVIDE_ADD_MARKER) {
            // std::uint32_t t = ((numer - q) >> 1) + q;
            // return t >> denom->shift;
            std::uint32_t shift = more & LIBDIVIDE_64_SHIFT_MASK;
...
__m512i libdivide_s32_do_vector(__m512i numers, const struct libdivide_s32_t *denom) {
    ...
    if (!denom->magic) {
        ...
        __m512i sign = _mm512_set1_epi32((std::int8_t)more >> 7);
        // q = (q ^ sign) - sign;
...
__m256i libdivide_s32_do_vector(__m256i numers, const struct libdivide_s32_t *denom) {
    ...
    if (!denom->magic) {
        ...
        __m256i sign = _mm256_set1_epi32((std::int8_t)more >> 7);
        // q = (q ^ sign) - sign;
...
__m128i libdivide_s32_do_vector(__m128i numers, const struct libdivide_s32_t *denom) {
    ...
    if (!denom->magic) {
        ...
        __m128i sign = _mm_set1_epi32((std::int8_t)more >> 7);
        // q = (q ^ sign) - sign;
...
// (like std::uint64_t) and an algorithm type.
...
template<> struct dispatcher<true, true, sizeof(std::int32_t), BRANCHFULL> { DISPATCHER_GEN(std::int32_t, s32) };
template<> struct dispatcher<true, true, sizeof(std::int32_t), BRANCHFREE> { DISPATCHER_GEN(std::int32_t, s32_branchfree) };
template<> struct dispatcher<true, false, sizeof(std::uint32_t), BRANCHFULL> { DISPATCHER_GEN(std::uint32_t, u32) };
template<> struct dispatcher<true, false, sizeof(std::uint32_t), BRANCHFREE> { DISPATCHER_GEN(std::uint32_t, u32_branchfree) };
template<> struct dispatcher<true, true, sizeof(std::int64_t), BRANCHFULL> { DISPATCHER_GEN(std::int64_t, s64) };
template<> struct dispatcher<true, true, sizeof(std::int64_t), BRANCHFREE> { DISPATCHER_GEN(std::int64_t, s64_branchfree) };
template<> struct dispatcher<true, false, sizeof(std::uint64_t), BRANCHFULL> { DISPATCHER_GEN(std::uint64_t, u64) };
template<> struct dispatcher<true, false, sizeof(std::uint64_t), BRANCHFREE> { DISPATCHER_GEN(std::uint64_t, u64_branchfree) };
