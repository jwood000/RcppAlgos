...
void StandardPartitions(int* mat, std::vector<int> &z, PartitionType ptype,
                        double lower, mpz_t lowerMpz, int nCols, int width,
                        int nRows, int nThreads, int lastCol, int lastElem,
                        int tar, int k, bool IsRep, bool IsMult,
                        bool IsGmp, bool IsComb, bool includeZero) {
    ...
    if (nThreads > 1) {
        ...
        for (int j = 0; j < (nThreads - 1);
            ...
            zs[j + 1] = nthPartFun(tar, width, k, lower, lowerMpz);
...
void GeneralPartitions(T* mat, const std::vector<T> &v, std::vector<int> &z,
                       ...
                       int lastElem, int k, bool IsComb) {
    ...
    if (nThreads > 1) {
        ...
        for (int j = 0; j < (nThreads - 1);
            ...
            threads.emplace_back(std::cref(PartsGenParallel<T>),
                                 ...
                                 std::ref(zs[j]), step, part.width, lastElem,
                                 lastCol, nextStep, part.isRep);
        ...
        threads.emplace_back(std::cref(PartsGenParallel<T>),
                             std::ref(parMat), std::cref(v),
                             std::ref(zs.back()), step, part.width, lastElem,
                             lastCol, nRows, part.isRep);

        for (auto& thr: threads) {
            thr.join();
        }
    } else {
        PartsGenManager(mat, part, v, z, part.width, lastElem,
