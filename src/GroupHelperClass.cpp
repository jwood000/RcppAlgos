...
bool GroupHelper::is_max(const std::vector<int> &z, int i) const {
...
GroupHelper::GroupHelper(const std::vector<int> &_grp,
...
bool GroupHelper::require_external(const std::vector<int> &z, int i) const {
    if (!same[i])     return false;
    if (is_max(z, i)) return false;
    return grp[i] != grp.back();
}

bool GroupHelper::flip_external(std::vector<int> &z, int &idx1, int i) const {
...
void GroupHelper::step(int &idx1, int &idx2, int &curr_bnd, int i) const {
    idx2     -= grp[i + 1];
    curr_bnd -= grp[i - 1];
    idx1     -= same[i] ? 2 : 1;
}

int GroupHelper::get_low(int curr_bnd, int i) const {
    return same[i] ? curr_bnd + 1 : curr_bnd;
}

int GroupHelper::get_size() const {
    return grp.size();
}
