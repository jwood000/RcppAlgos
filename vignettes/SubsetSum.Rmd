...
For example, using only the numbers `3:18`, find all partitions of length `5` of the `50`.
...
system.time(genParts2 <- comboGeneral(inpVec, 13, TRUE,
                                      ...
                                      limitConstraints = 2613))
 user  system elapsed 
0.209   0.002   0.211  ##  We still find over 100 thousand results out of
...
Although the above was about 14 times slower than the first example dealing with 573 million combinations (211 milliseconds vs. 16 milliseconds), we are still dealing in milliseconds!!! For reference, version `2.3.4` takes about 18 seconds to find all 118,560 solutions.
...
Oftentimes when working with numerical vectors, it can be hard to find combinations that sum to a particular number because of floating point errors (See [Using `tolerance`](<https://jwood000.github.io/RcppAlgos/articles/CombPermConstraints.html#using-tolerance>)).
