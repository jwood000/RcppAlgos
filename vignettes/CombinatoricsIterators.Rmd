...
#>  v243 20.81242 21.8859 23.3109 22.13436 22.4927 132.881   100
...
#> "as.environment"      3.46    32.58       3.46     32.58    3649.2
#> "$"                   3.18    29.94       8.04     75.71    8233.1
#> "test_nextIter"       1.28    12.05      10.62    100.00   10692.3
#> ".External"           0.96     9.04       0.96      9.04     936.2
#> "exists"              0.70     6.59       0.70      6.59     706.1
#> "get"                 0.66     6.21       0.66      6.21     709.6
#> 
#> $by.total
#>                 total.time total.pct mem.total self.time self.pct
#> "test_nextIter"      10.62       100   10692.3      1.28    12.05
#> "<Anonymous>"        10.62       100   10692.3      0.00     0.00
#> "block_exec"         10.62       100   10692.3      0.00     0.00
#> "call_block"         10.62       100   10692.3      0.00     0.00
#> "do.call"            10.62       100   10692.3      0.00     0.00
#> "doTryCatch"         10.62       100   10692.3      0.00     0.00
#> 
#> $sample.interval
#> [1] 0.02
#> 
...
#> Unit: milliseconds
#>    expr      min       lq     mean  median       uq      max neval
#>  curr_v 2.511906 2.652577 2.712456 2.68304 2.727792 4.522013   100
...
#> $by.self
#>                 self.time self.pct total.time total.pct mem.total
#> "<Anonymous>"        0.46    44.23       1.04    100.00     890.5
#> ".Call"              0.40    38.46       0.40     38.46     326.7
#> "test_nextIter"      0.16    15.38       1.02     98.08     870.6
#> "grepl"              0.02     1.92       0.02      1.92      19.9
#> 
#> $by.total
#>               total.time total.pct mem.total self.time self.pct
#> "<Anonymous>"       1.04       100     890.5      0.46    44.23
#> "block_exec"        1.04       100     890.5      0.00     0.00
#> "call_block"        1.04       100     890.5      0.00     0.00
#> "do.call"           1.04       100     890.5      0.00     0.00
#> "doTryCatch"        1.04       100     890.5      0.00     0.00
#> "eng_r"             1.04       100     890.5      0.00     0.00
#> 
#> $sample.interval
#> [1] 0.02
#> 
...
#>          1053412         17831624            28517         15142312         20461211 
#>        max.nodes     duplications tot.duplications          samples 
#>        194732384            18465          9805000              531
...
#> index: "tryCatch"
#>      vsize.small  max.vsize.small      vsize.large  max.vsize.large            nodes 
#>          3881681         28135720           302085         15672416         18269064 
#>        max.nodes     duplications tot.duplications          samples 
#>        189968240                3              140               52
```

With verison `2.5.0+` there are only 140 `tot.duplications` whereas with version `2.4.3` there are millions of `tot.duplications`. In fact, there are a total of 9805000 duplications with version `2.4.3`. This together with `comboCount(25, 10) = 3,268,760` implies that the C funciton, `duplicate`, is called about 3 times per iteration with older versions (i.e. 9805000 `/ 3268760 ~=` 2.9996).
...
Wow! Using the iterator approach is not only easier on your RAM, but faster as well (3.15 `/` 1.954 `~=` 1.6121)! Our gains came strictly from memory efficiency (From over 4 GBs to just over 1 MB) as the underlying algorithm is exactly the same.
