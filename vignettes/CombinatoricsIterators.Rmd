...
#>  v243 21.05953 22.14679 23.76128 22.53955 23.05418 127.3486   100
...
#> "as.environment"      3.64    34.47       3.64     34.47    1734.7
#> "$"                   3.00    28.41       8.16     77.27    4001.8
#> "test_nextIter"       1.14    10.80      10.56    100.00    4940.3
#> "get"                 0.84     7.95       0.84      7.95     419.5
#> ".External"           0.80     7.58       0.80      7.58     317.1
#> "exists"              0.66     6.25       0.66      6.25     281.4
#> 
#> $by.total
#>                             total.time total.pct mem.total self.time self.pct
#> "test_nextIter"                  10.56       100    4940.3      1.14     10.8
#> "<Anonymous>"                    10.56       100    4940.3      0.00      0.0
#> "base::do.call"                  10.56       100    4940.3      0.00      0.0
#> "base::saveRDS"                  10.56       100    4940.3      0.00      0.0
#> "base::tryCatch"                 10.56       100    4940.3      0.00      0.0
#> "base::withCallingHandlers"      10.56       100    4940.3      0.00      0.0
#> 
#> $sample.interval
#> [1] 0.02
#> 
...
#> Unit: milliseconds
#>    expr      min       lq     mean   median       uq      max neval
#>  curr_v 2.562049 2.619777 2.744743 2.686914 2.770083 4.235587   100
...
#> $by.self
#>                 self.time self.pct total.time total.pct mem.total
#> ".Call"              0.52    40.62       0.52     40.62     296.1
#> "<Anonymous>"        0.50    39.06       1.28    100.00     683.6
#> "test_nextIter"      0.24    18.75       1.26     98.44     683.5
#> "grepl"              0.02     1.56       0.02      1.56       0.0
#> 
#> $by.total
#>                             total.time total.pct mem.total self.time self.pct
#> "<Anonymous>"                     1.28       100     683.6       0.5    39.06
#> "base::do.call"                   1.28       100     683.6       0.0     0.00
#> "base::saveRDS"                   1.28       100     683.6       0.0     0.00
#> "base::tryCatch"                  1.28       100     683.6       0.0     0.00
#> "base::withCallingHandlers"       1.28       100     683.6       0.0     0.00
#> "block_exec"                      1.28       100     683.6       0.0     0.00
#> 
#> $sample.interval
#> [1] 0.02
#> 
...
#>           484198          5441872            13551          7154768          9446675 
#>        max.nodes     duplications tot.duplications          samples 
#>         57745352            18567          9803476              528
...
#> index: "base::tryCatch"
#>      vsize.small  max.vsize.small      vsize.large  max.vsize.large            nodes 
#>          2171456          5625568           113538          7230424          9886475 
#>        max.nodes     duplications tot.duplications          samples 
#>         49337904                2              142               64
```

With verison `2.5.0+` there are only 142 `tot.duplications` whereas with version `2.4.3` there are millions of `tot.duplications`. In fact, there are a total of 9803476 duplications with version `2.4.3`. This together with `comboCount(25, 10) = 3,268,760` implies that the C funciton, `duplicate`, is called about 3 times per iteration with older versions (i.e. 9803476 `/ 3268760 ~=` 2.9991).
...
Wow! Using the iterator approach is not only easier on your RAM, but faster as well (4.987 `/` 1.567 `~=` 3.1825)! Our gains came strictly from memory efficiency (From over 4 GBs to just over 1 MB) as the underlying algorithm is exactly the same.
