...
#>  test_nextIter(15, 8) 18.96939 20.49509 22.8305 22.2211 24.70213 31.21137   100
...
#> "$"                  4.14    40.75       4.14     40.75    3077.7
#> "a$nextIter"         1.64    16.14       4.40     43.31    3534.4
#> "test_nextIter"      1.62    15.94      10.16    100.00    7840.6
#> "slot"               1.32    12.99       1.32     12.99    1003.9
#> "<Anonymous>"        0.98     9.65       1.44     14.17    1297.6
#> ".Call"              0.46     4.53       0.46      4.53     397.5
#>
#> $by.total
#>                 total.time total.pct mem.total self.time self.pct
#> "test_nextIter"      10.16    100.00    7840.6      1.62    15.94
#> "a$nextIter"          4.40     43.31    3534.4      1.64    16.14
#> "$"                   4.14     40.75    3077.7      4.14    40.75
#> "<Anonymous>"         1.44     14.17    1297.6      0.98     9.65
#> "slot"                1.32     12.99    1003.9      1.32    12.99
#> ".Call"               0.46      4.53     397.5      0.46     4.53
#>
#> $sample.interval
#> [1] 0.02
#>
...
#> Unit: milliseconds
#>                           expr      min       lq     mean  median       uq    max neval
#>  test_nextIter(15, 8, v = 251) 3.324678 3.462249 3.755247 3.64253 3.856856 9.4942   100
...
#> $by.self
#>                 self.time self.pct total.time total.pct mem.total
#> "<Anonymous>"        0.82    50.00       1.22     74.39     591.4
#> "test_nextIter"      0.42    25.61       1.64    100.00     828.1
#> ".Call"              0.40    24.39       0.40     24.39     208.5
#>
#> $by.total
#>                 total.time total.pct mem.total self.time self.pct
#> "test_nextIter"       1.64    100.00     828.1      0.42    25.61
#> "<Anonymous>"         1.22     74.39     591.4      0.82    50.00
#> ".Call"               0.40     24.39     208.5      0.40    24.39
#>
#> $sample.interval
#> [1] 0.02
#>
...
#>           818856          1528208                0                0         15084023         28150864            12915          1046139               81
#> ------------------------------------------------------------------------------------------------------------------------------------------------------
#> index: "test_nextIter":"$"
#>      vsize.small  max.vsize.small      vsize.large  max.vsize.large            nodes        max.nodes     duplications tot.duplications          samples
#>           893682         18816032            92410         19128912         15580492        164092096            12582          2604432              207
#> ------------------------------------------------------------------------------------------------------------------------------------------------------
#> index: "test_nextIter":"a$nextIter"
#>      vsize.small  max.vsize.small      vsize.large  max.vsize.large            nodes        max.nodes     duplications tot.duplications          samples
#>           867378          1534896                0                0         15978375         28274568            13087          2879232              220

## Version 2.5.0
summaryRprof("Version250.out", memory = "stats")
#> index: "test_nextIter"
#>      vsize.small  max.vsize.small      vsize.large  max.vsize.large            nodes        max.nodes     duplications tot.duplications          samples
#>          2626749          3953536                0                0          9193563         13837376                0                0               21
#> ---------------------------------------------------------------------------------------------------------------------------------------------------
#> index: "test_nextIter":"<Anonymous>"
#>      vsize.small  max.vsize.small      vsize.large  max.vsize.large            nodes        max.nodes     duplications tot.duplications          samples
#>          2533896         17629656           277772         16944064         10035058        132854400                0                7               61
```

With verison `2.5.0` there are only 7 `tot.duplications` whereas with version `2.4.3` there are millions of `tot.duplications`. In fact, there are a total of `1046139 + 2604432 + 2879232 = 6,529,803` duplications with version `2.4.3`. This together with `comboCount(25, 10) = 3,268,760` implies that the C funciton, `duplicate`, is called about 2 times per iteration with older versions (i.e. `6529803 / 3268760 ~= 1.997639`).
