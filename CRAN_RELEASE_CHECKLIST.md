...
Confirm you are on a clean release branch (no WIP for the next large
release).
...
Ensure `inst/CITATION` contains **no dynamic calls** that require the
package to be installed/available (e.g., no
...
[`citation()`](https://rdrr.io/r/utils/citation.html), etc.).
...
Permanent redirects (301) by switching to canonical URL (often add
trailing slash)
...
Verify: - \[ \] No ERRORs or WARNINGs. - \[ \] Any NOTEs are
understood/acceptable (avoid new NOTES if possible). - \[ \] In
particular: - \[ \] CITATION readable in incoming feasibility checks -
\[ \] No invalid URLs (especially malformed GitHub links) - \[ \] No new
...
If pkgdown warns about missing alt text, consider fixing in
README/vignettes.
...
This is a quick way to catch undefined behavior (UB) that may not show
up in regular checks. Uses the rocker image:
...
- Use `--no-examples --no-vignettes` for speed. Remove these flags when
  debugging vignette/example-related issues.
- If you are investigating memory/UB issues deeply, consider running
  without `--no-vignettes` to widen coverage.

------------------------------------------------------------------------

## Debugging UBSan traces (addr2line)

Sometimes UBSan prints stack frames with offsets into the shared object,
e.g.:
